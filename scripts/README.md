...
| Script | Purpose |
| ------ | ------- |
| `setup.sh` | One-command environment setup: finds Python 3.11+, creates `.venv`, installs `ai-journal-mcp[dev,server]` editable, installs the pre-commit hook. Idempotent. |
